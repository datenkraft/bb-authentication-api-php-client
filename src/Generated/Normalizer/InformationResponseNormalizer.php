...
class InformationResponseNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return $type === \Datenkraft\Backbone\Client\AuthenticationApi\Generated\Model\InformationResponse::class;
    }
    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return is_object($data) && get_class($data) === \Datenkraft\Backbone\Client\AuthenticationApi\Generated\Model\InformationResponse::class;
    }
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('information', $data)) {
            ...
            foreach ($data['information'] as $value) {
                $values[] = $this->denormalizer->denormalize($value, \Datenkraft\Backbone\Client\AuthenticationApi\Generated\Model\Information::class, 'json', $context);
            }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('information') && null !== $data->getInformation()) {
            $values = [];
            foreach ($data->getInformation() as $value) {
                $values[] = $this->normalizer->normalize($value, 'json', $context);
            }
            $dataArray['information'] = $values;
        }
        foreach ($data as $key => $value_1) {
            if (preg_match('/.*/', (string) $key)) {
    ...
    {
        return [\Datenkraft\Backbone\Client\AuthenticationApi\Generated\Model\InformationResponse::class => false];
    }
}